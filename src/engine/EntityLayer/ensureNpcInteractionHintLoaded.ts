...
export function ensureNpcInteractionHintLoaded(layer: IEntityLayer, npc: NPC): void {
  ...
  void convex
    ...
    .then((profile: { tags?: string[]; instanceType?: string } | null) => {
      const hostile = !!profile && Array.isArray(profile.tags) && profile.tags.includes("hostile");
